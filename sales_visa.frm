...
Begin VB.Form salesfrm 
   ...
   Begin VB.CommandButton cmdSendSales 
      Caption         =   "«—”«· „»Ì⁄« "
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   11.25
   ...
   Begin VB.Frame Frame6 
      ...
      Begin Threed.SSCommand cmdLast 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "sales_visa.frx":2C43F
         Caption         =   "«ŒÌ—"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "sales_visa.frx":2E60F
      End
      Begin Threed.SSCommand cmdNext 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "sales_visa.frx":30757
         Caption         =   "·«Õﬁ "
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "sales_visa.frx":3291F
      End
      Begin Threed.SSCommand cmdPrevious 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "sales_visa.frx":34A6E
         Caption         =   "”«»ﬁ"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "sales_visa.frx":36C4E
      End
      Begin Threed.SSCommand cmdFirst 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "sales_visa.frx":38DA9
         Caption         =   "√Ê·"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "sales_visa.frx":3AF65
      End
   End
...
Private Sub CND_PRICE3_Click()
    If InputBox("", "") = "987654321" Then
        con.Execute "UPDATE FILE6_20 SET  file6_20.price = file1_10.cost2 , file6_20.price_C = file1_10.PRICE  FROM   FILE6_20 INNER JOIN FILE1_10 ON FILE6_20.ITEM = FILE1_10.ITEM where DOC_NO = " & MyParn(xdoc_no.text)
        myLoadGrd
    End If
...
Dim oCash As New cash_salesfrm
oCash.nTotal = xtotal.text
oCash.bEditRecord = xClosed.Value = 0 Or bopt2
oCash.sDoc_no = xdoc_no.text
Set oCash.myForm = Me
oCash.Show 1
End Function
Private Function reSendSales() As Boolean
...
'    Dim cString As String
'    Dim nCountUndone As Integer
'    cString = "SELECT COUNT(*) AS ACOUNT_OF " & _
'               " FROM ACCOUNT_H INNER JOIN FILE0_50 ON ACCOUNT_H.BOX = FILE0_50.CODE" & _
'               " WHERE FILE0_50.BRANCH = " & MyParn(cBranch) & _
'               " AND ACCOUNT_H.DONE = 0"
'    nCountUndone = Val(myField(cString, con))
'    If nCountUndone > 0 Then
'        MsgBox "⁄œœ " & nCountUndone & " „” ‰œ«  ÌÊ„Ì… Œ“‰ €Ì— „‰ ÂÌ"
'        Exit Sub
'    End If
'End If
...
If cBranch <> "00" And (XSALES_RET.Caption = "") And Not lISNOGRDISC And xcard_disc.text = "" Then
    If XUSERNAME_DISC.Caption = "" Then
        ...
        If nDiscBranch > 0 And IsDate(dDiscBranch1) And IsDate(dDiscBranch2) And xdoc_disc.Caption = "" Then
...
If DateValue(salesfrm.xdate.text) = DateValue(dSalesDate) Then
    ...
    If IsDate(dDate) Then
        ...
        If MsgBox(CD, vbOKCancel + vbDefaultButton2) = vbOK Then
            ...
            If cBranch <> "00" And lServerOnLine Then
                Dim nAffect As Long
...
Do Until loctable.EOF
    ...
    conShop.Execute "delete from file6_20_VISA where doc_no = " & MyParn(loctable!doc_no)
...
'    aInsert = AddFlag(aInsert, "[BOXVISA]", addstring(loctable!BOXVISA))
    ...
    aInsert = AddFlag(aInsert, "[VISA3_2]", mRound(loctable!VISA3_2))
...
'    cStr1 = cStr1 & addInsert(aInsert, cFile & "H") & " ; "
    conShop.Execute addInsert(aInsert, "FILE6_20H")
    lSkip = False
    ...
    locTableSub.Open cString, con, adOpenStatic, adLockReadOnly, adCmdText
    ...
    Do Until locTableSub.EOF
        ...
        If GetDesca("select item from file1_10 where  item = " & sitem, conShop) <> "" Then
            aInsert2 = AddFlag(Empty, "DOC_NO", addstring(locTableSub!doc_no))
            aInsert2 = AddFlag(aInsert2, "ITEM", addvalue(locTableSub!Item))
            aInsert2 = AddFlag(aInsert2, "MAN", addstring(locTableSub!MAN))
            aInsert2 = AddFlag(aInsert2, "QUANT", Val(locTableSub!Quant & ""))
            aInsert2 = AddFlag(aInsert2, "PRICE", Val(locTableSub!price & ""))
            aInsert2 = AddFlag(aInsert2, "DISCOUNT", Val(locTableSub!discount & ""))
            aInsert2 = AddFlag(aInsert2, "PRICE_C", Val(locTableSub!PRICE_C & ""))
            aInsert2 = AddFlag(aInsert2, "ROW", Val(locTableSub!Row & ""))
            aInsert2 = AddFlag(aInsert2, "COST", Val(locTableSub!cost & ""))
            aInsert2 = AddFlag(aInsert2, "S_OKAZ", Val(locTableSub!S_OKAZ & ""))
            aInsert2 = AddFlag(aInsert2, "ISDISC2_1", IIf(locTableSub!ISDISC2_1, 1, 0))
...
'            MsgBox " „—«Ã⁄… „»Ì⁄«  »«—ﬂÊœ " & sitem
    ...
    Set locTableSub = Nothing
    Set locTableSub = New ADODB.Recordset
    Set locTableSub = mycmd("Select * from FILE6_20_VISA WHERE doc_no = " & MyParn(loctable!doc_no), con)
    Do Until locTableSub.EOF
        ...
        aInsert = AddFlag(aInsert, "[APPROVE_NO]", locTableSub!approve_no)
        sb.Append addInsert(aInsert, "FILE6_20_VISA") & ";"
        locTableSub.MoveNext
    Loop
    
    If sb.Length <> 0 Then
        conShop.Execute sb.GetAsString()
    End If
