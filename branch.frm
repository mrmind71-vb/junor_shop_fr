...
Begin VB.Form branch 
   ...
   ScaleHeight     =   3150
   ScaleWidth      =   7185
   Begin VB.CommandButton SHOW_CONF 
      Caption         =   "⁄—÷ ﬂÊœ «·›—⁄ ·„·› «· CONF"
      Height          =   330
      Left            =   855
      RightToLeft     =   -1  'True
...
CardTable.Open "SELECT * FROM BRANCH ORDER BY CODE", con, adOpenStatic, adLockReadOnly, adCmdText
...
If MsgBox("«·€«¡ «·”Ã· «·Õ«·Ï : Â· «‰  „Ê«›ﬁ ø", 4) = 6 Then
    con.BeginTrans
    con.Execute "Delete  From BRANCH  Where code = " & MyParn(xCode.text)
    con.CommitTrans
    CardTable.Requery
    If Not (CardTable.EOF And CardTable.BOF) Then
...
If xstore.BoundText <> "" Then
    If MsgBox("  ÕœÌœ «·›—⁄ ··„Œ“‰ ", vbYesNo) = vbYes Then
        con.Execute " update file0_40 set branch = " & addstring(xCode.text) & " where code = " & MyParn(xstore.BoundText)
    End If
End If
...
Generalarray(1) = "Select code ,DescA From BRANCH"
...
End Sub
Sub myDefine()
xCode.text = IncRec(GetDesca("SELECT MAX(CODE) FROM BRANCH WHERE CODE < '100' ", con))
xDescA.text = ""
xstore.BoundText = ""
xipsql.text = ""
xisstop.Value = 0
...
xisbranch2.Value = IIf(CardTable!isbranch2, 1, 0)
...
aInsert(3, 0) = "IPSQL"
aInsert(3, 1) = addstring(xipsql.text)

aInsert(4, 0) = "ISSTOP"
aInsert(4, 1) = IIf(xisstop.Value, 1, 0)

aInsert(5, 0) = "isbranch2"
aInsert(5, 1) = IIf(xisbranch2.Value, 1, 0)

aInsert(6, 0) = "isbranch"
aInsert(6, 1) = IIf(XISBRANCH.Value, 1, 0)

On Error GoTo myerror
con.BeginTrans
If xCode.Enabled Then
    aInsert(0, 1) = addstring(xCode.text)
    con.Execute CreateInsert(aInsert, "BRANCH")
Else
    con.Execute CreateUpdate(aInsert, "BRANCH", " where CODE = " & addstring(xCode.text))
End If
con.CommitTrans
myreplace = True
Exit Function
...
closeCon con
End Sub
Private Sub SHOW_CONF_Click()
    InputBox "", "ﬂÊœ «·›—⁄", crypt(xCode, "dr")
End Sub
