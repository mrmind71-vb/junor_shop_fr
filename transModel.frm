...
If bCost1 Then FixCost cFieldas, cField
Fixgrd
CalcTotals
myloadgrd = True
End Function
...
cString = "Select " & cFieldas & _
          " From " & _
          " (Select scal,cost from file1_10 WHERE MODEL = " & MyParn(xModel2.Caption) & " ) AS TABLE1" & _
          " PIVOT " & _
          " (max(cost)" & _
          " FOR SCAL IN " & _
          "(" & cField & ")" & _
...
cString = "Select " & cFieldas & _
          " From " & _
          " (Select scal,cost2 from file1_10 WHERE MODEL = " & MyParn(xModel2.Caption) & " ) AS TABLE1" & _
          " PIVOT " & _
          " (max(cost2)" & _
          " FOR SCAL IN " & _
          "(" & cField & ")" & _
...
'            nFoundRow = myForm.grid1.FindRow(GRDTABLE.Fields(nCol).Value & "", , nColItem)
            nQ = GetDesca("SELECT SUM(QUANT) FROM FILE1_60 WHERE DOC_NO = " & MyParn(Transfrm.xdoc_no.text) & " AND ITEM = " & Val(GrdTable.Fields(nCol).Value & ""), con)
'            If nFoundRow <> -1 Then
'                 grid1.TextMatrix(grid1.Rows - 1, nCol) = Myvalue(myForm.grid1.TextMatrix(nFoundRow, nColQuant))
'             Else
                 grid1.TextMatrix(grid1.Rows - 1, nCol) = nQ
'             End If
        End If
    Next
    GrdTable.MoveNext
