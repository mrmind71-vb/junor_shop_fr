...
Begin VB.Form ship_codesfrm 
   ...
   End
End
Attribute VB_Name = "ship_codesfrm"
...
aInsert = AddFlag(aInsert, "[STOPED]", IIf(.ValueMatrix(Row, 2) = 0, 0, 1))

On Error GoTo myerror
If .TextMatrix(Row, 0) = "" Then
    .TextMatrix(Row, 0) = RetZero(Newflag("SHIP", "CODE", con), 2)
    aInsert = AddFlag(aInsert, "CODE", addstring(.TextMatrix(Row, 0)))
    con.Execute addInsert(aInsert, "SHIP")
Else
    con.Execute addUpdate(aInsert, "SHIP", "CODE = " & addstring(.TextMatrix(Row, 0)))
End If
End With
Exit Sub
myerror:
    ...
    If Trim(grid1.TextMatrix(grid1.Row, 0)) <> "" Then
        If MsgBox("Õ–›?? Â· √‰  „Ê«›ﬁ", vbYesNo + vbDefaultButton2 + vbCritical, "Õ–›") = vbYes Then
            If IsNumeric(grid1.TextMatrix(grid1.Row, 0)) Then
                con.BeginTrans
                con.Execute "Delete From SHIP where code =  " & MyParn(grid1.TextMatrix(grid1.Row, 0))
                con.CommitTrans
            End If
