...
Set loctable = ModelFind(xModel.Caption, con)
If Not (loctable.EOF And loctable.BOF) Then
    ...
    xRemark.Text = GetDesca("SELECT REMARK FROM " & Purchasefrm.cFile & " WHERE DOC_NO = " & MyParn(Purchasefrm.xDoc_No.Text) & " AND MODEL = " & MyParn(xModel.Caption))
    myLoadModelgrd
    If Purchasefrm.myPublic = 0 Or Purchasefrm.myPublic = 1 Then
        myloadgrd2
    End If
...
If KeyAscii = 13 And xbarcode.Text <> "" Then
    aRet = aGetDesca("select model , modelfact , fact , mosm from file1_10 where code = " & MyParn(Purchasefrm.xCode.Text) & " and item = " & Val(xbarcode.Text))
...
If Not xFact.MatchedWithList Then
    ...
    cCode = IncRec(GetDesca("SELECT MAX(CODE) FROM FACT WHERE CODE > '300' "))
...
If xMosm.MatchedWithList And xFact.MatchedWithList And Trim(xModelFact.Text) <> "" Then
    ...
    If GetDesca("select model from file1_10 where model = " & MyParn(xModel.Caption)) = "" Then
        sSubMosm = GetDesca("select mosm from file1_10 where fact = " & MyParn(xFact.BoundText) & " and modelfact = " & MyParn(xModelFact.Text))
        If sSubMosm <> "" Then
