...
If Trim(xCode.text) <> "" Then
    cWhere = cWhere & turnFound(cWhere) & " FILE6_20h.code = " & MyParn(xCode.text)
...
If IsDate(xdate1.text) Then
    cWhere = cWhere & turnFound(cWhere, " and ") & " FILE6_20H.date >= " & DateSq(xdate1.text)
    aHeader(2) = "[" & BetweenString(xdate1.text, xDate2.text) & "]"
End If

If IsDate(xDate2.text) Then
    cWhere = cWhere & turnFound(cWhere, " and ") & " FILE6_20H.date <= " & DateSq(xDate2.text)
    aHeader(2) = "[" & BetweenString(xdate1.text, xDate2.text) & "]"
End If

If Trim(XSTORE.BoundText) <> "" Then
    cWhere = cWhere & turnFound(cWhere, " and ") & " FILE6_20h.STORE = " & MyParn(XSTORE.BoundText)
    aHeader(3) = "[" & "·„Œ“‰ : " & XSTORE.text & "]"
End If

'If chkPaid(0).Value = 1 Then
'    cWhere = cWhere & turnFound(cWhere, " and ") & " INVPAIDSUM.REST > 0 "
'    aHeader(3) = "[" & "«·›Ê« Ì— «·€Ì— „”œœ… ›ﬁÿ" & "]"
'End If
'
...
cField4 = "(Select Sum(invpaidSum.Pay) " & _
          " from (FILE6_20h LEFT JOIN INVPAIDSUM ON FILE6_20H.DOC_NO = INVPAIDSUM.DOC_NO) left join FILE3_10 on FILE6_20h.code = FILE3_10.code " & _
          turnFound(cWhere) & _
          cWhere & ") as SumOfPay"
                               
'cField5 = "(Select Sum(invpaidSum.Rest) " & _
'          " from (FILE6_20h LEFT JOIN INVPAIDSUM ON FILE6_20H.DOC_NO = INVPAIDSUM.DOC_NO) left join FILE3_10 on FILE6_20h.code = FILE3_10.code " & _
'          turnFound(cWhere) & _
'          cWhere & ") as SumOfRest"
                               
'cField6 = "(Select Sum(invpaidSum.Cash) " & _
'          " from (FILE6_20h LEFT JOIN INVPAIDSUM ON FILE6_20H.DOC_NO = INVPAIDSUM.DOC_NO) left join FILE3_10 on FILE6_20h.code = FILE3_10.code " & _
'          turnFound(cWhere) & _
'          cWhere & ") as SumOfCash"
                               
'cField7 = "(Select Sum(invpaidSum.Visa) " & _
'          " from (FILE6_20h LEFT JOIN INVPAIDSUM ON FILE6_20H.DOC_NO = INVPAIDSUM.DOC_NO) left join FILE3_10 on FILE6_20h.code = FILE3_10.code " & _
'          turnFound(cWhere) & _
'          cWhere & ") as SumOfVisa"
                               
'cField8 = "(Select Sum(invpaidSum.Post) " & _
'          " from (FILE6_20h LEFT JOIN INVPAIDSUM ON FILE6_20H.DOC_NO = INVPAIDSUM.DOC_NO) left join FILE3_10 on FILE6_20h.code = FILE3_10.code " & _
'          turnFound(cWhere) & _
'          cWhere & ") as SumOfPost"
