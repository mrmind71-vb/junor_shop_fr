...
Begin VB.Form VsTFRBranshSalSupl 
   Caption         =   "„ﬁ«—‰… „»Ì⁄«  «·„Ê—œÌ‰ ·· ÊﬂÌ·« "
   ClientHeight    =   10365
   ClientLeft      =   75
   ClientTop       =   450
   ClientWidth     =   15240
   ...
   Begin VB.Frame Frame4 
      ...
      Begin VB.CommandButton CMD_PRINT 
         Height          =   465
         Left            =   1260
         Picture         =   "VsTFRBranshSalSupl.frx":0000
         RightToLeft     =   -1  'True
         Style           =   1  'Graphical
         TabIndex        =   19
         Top             =   135
      ...
      Begin VB.CommandButton cmdExit 
         Height          =   465
         Left            =   45
         Picture         =   "VsTFRBranshSalSupl.frx":242A
         RightToLeft     =   -1  'True
         Style           =   1  'Graphical
         TabIndex        =   18
         Top             =   135
      ...
      Begin VB.CommandButton cmdGo 
         Height          =   465
         Left            =   2475
         Picture         =   "VsTFRBranshSalSupl.frx":4896
         RightToLeft     =   -1  'True
         Style           =   1  'Graphical
         TabIndex        =   17
         ToolTipText     =   "⁄—÷"
      ...
      Begin Threed.SSCommand cmd_excel 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "VsTFRBranshSalSupl.frx":6D88
         Caption         =   " ÕÊÌ· «ﬂ”Ì· "
         Alignment       =   4
         PictureAlignment=   1
      End
   ...
   Begin VSFlex7Ctl.VSFlexGrid grid1 
      Bindings        =   "VsTFRBranshSalSupl.frx":9583
      Height          =   7380
      Left            =   135
      TabIndex        =   1
      Top             =   1890
   ...
   End
End
Attribute VB_Name = "VsTFRBranshSalSupl"
...
Do Until StoreTable.EOF
    ...
    cField2 = cField2 & turnFound(cField2, ",") & myiif(cWhere, "(TOTAL * ((100-FR_SALES_MODEL.RATE)/100))")
    StoreTable.MoveNext
Loop

With GRID1
'                           0               1                 2                3               4                        5
    cStrall = " SELECT     FR_SALES_MODEL.CODE, FR_SALES_MODEL.SUPPDESCA, SUM(QUANT) , SUM(TOTAL * ((100-FR_SALES_MODEL.RATE)/100))  , " & _
                cField2 & _
            " FROM  FR_SALES_MODEL INNER JOIN FILE1_10 ON FILE1_10.ITEM = FR_SALES_MODEL.ITEM WHERE FR_SALES_MODEL.CODE IS NOT NULL  "
    If xMosm.BoundText <> "" Then cStrall = cStrall & " AND FR_SALES_MODEL.MOSM = " & MyParn(xMosm.BoundText)
    If xSupp.BoundText <> "" Then cStrall = cStrall & " AND FR_SALES_MODEL.CODE = " & MyParn(xSupp.BoundText)
    If xGroup.BoundText <> "" Then cStrall = cStrall & " AND FR_SALES_MODEL.[GROUP] = " & MyParn(xGroup.BoundText)
    If xFact.BoundText <> "" Then cStrall = cStrall & " AND FR_SALES_MODEL.FACT = " & MyParn(xFact.BoundText)
    If XSECTION.BoundText <> "" Then cStrall = cStrall & " AND FILE1_10.[Section] = " & XSECTION.BoundText
    If IsDate(xdate1.text) Then cStrall = cStrall & " AND FR_SALES_MODEL.[DATE] >= " & DateSq(xdate1.text)
    If IsDate(xdate1.text) Then cStrall = cStrall & " AND FR_SALES_MODEL.[DATE] <= " & DateSq(xdate2.text)
    cStrall = cStrall & " GROUP BY  FR_SALES_MODEL.CODE, FR_SALES_MODEL.SUPPDESCA ORDER BY FR_SALES_MODEL.CODE "
