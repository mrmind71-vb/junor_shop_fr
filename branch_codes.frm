...
Begin VB.Form branch_codesfrm 
   ...
   End
End
Attribute VB_Name = "branch_codesfrm"
...
Private Sub myload()
'                   0                   1                       2                   3               4                               5
cString = "SELECT CODE as [«·ﬂÊœ], DESCA as [«·›—⁄], [STORE] as [«·„Œ“‰] , IPSQL AS [IP Number] , ISOKAZ  AS [”⁄— «Êﬂ«“ÌÊ‰ ] , ISOKAZMODEL AS [”⁄— «Êﬂ«“ÊÌ‰ ·„ÊœÌ·«  „Õœœ…] " & _
          " FROM BRANCH WHERE CODE <> '00' AND ISSTOP = 0 "
...
aInsert = AddFlag(aInsert, "[STORE]", addstring(.TextMatrix(Row, 2)))
aInsert = AddFlag(aInsert, "[IPSQL]", addstring(.TextMatrix(Row, 3)))

aInsert = AddFlag(aInsert, "[ISOKAZ]", Val(.TextMatrix(Row, 4)))
aInsert = AddFlag(aInsert, "[ISOKAZMODEL]", Val(.TextMatrix(Row, 5)))
...
If Not IsNumeric(.TextMatrix(Row, 0)) Then
    ...
    con.Execute addInsert(aInsert, "BRANCH")
Else
    con.Execute addUpdate(aInsert, "BRANCH", "code = " & MyParn(.TextMatrix(Row, 0)))
End If
con.CommitTrans
End With
Exit Sub
...
'                con.BeginTrans
'                con.Execute "Delete From FILE0_40 where code =  " & grid1.TextMatrix(grid1.Row, 0)
'                con.CommitTrans
'            End If
...
With grid1
clist1 = StrList("Select code,desca from FILE0_40 ORDER BY CODE", con)
myload
...
End Function
Private Function LoadConString_shop(nRow)
With grid1
    cServerName = .TextMatrix(nRow, 3)
    cUserId = decrypt(RetSetting("userId", App.Path & "\conf.txt"), "dr")
    cPassword = decrypt(RetSetting("Password", App.Path & "\conf.txt"), "dr")
    sCatalog = GetDesca("SELECT DATA FROM BRANCH WHERE CODE = " & MyParn(.TextMatrix(nRow, 0)), con)
    closeCon con
    openCon con
    aBranch = GetFields("select code from branch", con)
    sBranchCode = GetDesca("select code from branch", con)
    cBranch = sBranchCode
    sBranch = sBranchCode
    main.Show
End With
