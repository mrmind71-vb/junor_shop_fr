...
Private Sub MYLOAD2()
    ...
    For nRow = 4 To .Rows - 1
        For nCol = 2 To .Cols - 1
            .TextMatrix(nRow, nCol) = Val(GetDesca("SELECT SUM([IN]-[OUT]) FROM FILE1_10 INNER JOIN FR1_11 ON FR1_11.ITEM = FILE1_10.ITEM  WHERE STORE = " & MyParn(pstore) & " AND MODEL = " & MyParn(pModelNo) & " AND SCAL = " & MyParn(.TextMatrix(0, nCol)) & " AND COLOR = " & MyParn(.TextMatrix(nRow, 1)), con) & "")
        Next nCol
    Next nRow
    End With
    CalcTotals2
