...
If xCode.text = "" Or xMosm.Caption = "" Then
    MsgBox "„—«Ã⁄… ﬂÊœ «·„Ê—œ Ê «·„Ê”„"
    Exit Sub
End If
cString = "select FILE4_11.*,FILE4_12.desca as MoveDesca  " & _
          " From FILE4_11 left join FILE4_12 on FILE4_11.[type] = FILE4_12.code "
cString = cString & turn(cString) & " FILE4_11.code = " & MyParn(xCode.text)
cString = cString & turn(cString) & " FILE4_11.MOSM = " & MyParn(xMosm.Caption)
...
cString = cString & " order by MYINDEX , Date,FILE4_12.[ORDER],FILE4_11.SAL,file4_11.doc_id"
...
invGrid.ColComboList(8) = StrListA("SELECT * from BRANCH ", con)
...
cPhone = GetDesca("SELECT PHONE1 FROM FILE4_10 WHERE CODE = " & MyParn(xCode.text), con)
cPhone = cPhone & " " & GetDesca("SELECT FAX FROM FILE4_10 WHERE CODE = " & MyParn(xCode.text), con)
Dim nBal As Double
...
n2 = Val(GetDesca("SELECT SUM(TAX ) FROM FILE7_20H WHERE " & cWhere, con) & "")
n3 = Val(GetDesca("SELECT SUM(DISCOUNT) FROM FILE7_20H WHERE " & cWhere, con) & "")
n4 = Val(GetDesca("SELECT SUM(SAL)      FROM FILE4_11  WHERE " & cWhere & " AND [TYPE] = '4' ", con) & "")
n1 = n4 - n2 + n3

n6 = Val(GetDesca("SELECT SUM(TAX ) FROM FILE7_10H WHERE " & cWhere, con) & "")
n7 = Val(GetDesca("SELECT SUM(DISCOUNT) FROM FILE7_10H WHERE " & cWhere, con) & "")
n8 = Val(GetDesca("SELECT SUM(pay)      FROM FILE4_11  WHERE " & cWhere & " AND [TYPE] = '5' ", con) & "")
n5 = n8 - n6 + n7
 
n9 = Val(GetDesca("SELECT SUM(PAY -SAL )       FROM FILE4_11  WHERE " & cWhere & " AND [TYPE] > '5' ", con) & "")
n10 = Val(GetDesca("SELECT SUM(SAL -PAY )       FROM FILE4_11  WHERE " & cWhere, con) & "")
...
cMosmDesca = GetDesca("SELECT DESCA FROM MOSM WHERE MOSM = " & MyParn(xMosm.Caption), con)
With sourcetable
    Do While Not .EOF
        temptable.AddNew
...
'   On Error GoTo myerror
    ...
    Else
        If GetDesca("SELECT CODE FROM FILE4_22 WHERE MOSM = " & MyParn(cPMosm) & " AND CODE = " & MyParn(xCode.text), con) = "" Then
'            Inform "1"
            dFdate = GetDesca("SELECT DATE FROM MOSM WHERE MOSM = " & MyParn(cPMosm), con)
'            Inform "2"
...
'cStr1 = " INSERT INTO file4_22 (code, mosm, DATE, f_bal) SELECT     FILE4_11.code, FILE4_11.mosm, GETDATE() AS Expr1, 0 AS Expr2 " & _
'        " FROM         FILE4_10 INNER JOIN FILE4_11 ON FILE4_10.CODE = FILE4_11.code LEFT OUTER JOIN file4_22 AS file4_22_1 ON FILE4_11.mosm = file4_22_1.mosm AND FILE4_11.code = file4_22_1.code " & _
'        " WHERE     (file4_22_1.code IS NULL) AND (FILE4_11.code IS NOT NULL) and file4_11.mosm is not  nullGROUP BY FILE4_11.code, FILE4_11.mosm "
'CON.Execute cStr1

cStr1 = " INSERT INTO file4_22 (code, mosm, DATE, f_bal) SELECT     FILE4_11.code, FILE4_11.mosm, GETDATE() AS Expr1, 0 AS Expr2 " & _
        " FROM         FILE4_10 INNER JOIN FILE4_11 ON FILE4_10.CODE = FILE4_11.code LEFT OUTER JOIN file4_22 AS file4_22_1 ON FILE4_11.mosm = file4_22_1.mosm AND FILE4_11.code = file4_22_1.code " & _
        " WHERE     (file4_22_1.code IS NULL) AND (FILE4_11.code IS NOT NULL) and file4_11.mosm is not  null GROUP BY FILE4_11.code, FILE4_11.mosm "
con.Execute cStr1

cStr1 = "DELETE FROM file4_22 FROM  FILE4_11 RIGHT OUTER JOIN file4_22 ON FILE4_11.mosm = file4_22.mosm AND FILE4_11.code = file4_22.code Where file4_11.code Is Null "
con.Execute cStr1
...
Private Sub xcode_Validate(Cancel As Boolean)
    If xCode.text = "" Then Exit Sub
    If GetDesca("SELECT CODE FROM FILE4_10 WHERE code = " & MyParn(xCode.text), con) = "" Then
        cCode = GetDesca("SELECT CODE FROM FILE4_10 WHERE SUBCODE = " & MyParn(xCode.text), con)
        xCode.text = cCode
    End If
End Sub
...
Sub FillTot()
    ...
    nV1 = Val(GetDesca("SELECT sum(DISCOUNT) FROM FILE7_20H WHERE CODE = " & MyParn(xCode.text) & " AND MOSM = " & MyParn(xMosm.Caption), con) & "")
    nV2 = Val(GetDesca("SELECT sum(DISCOUNT) FROM FILE7_10H WHERE CODE = " & MyParn(xCode.text) & " AND MOSM = " & MyParn(xMosm.Caption), con) & "")
    nV3 = Val(GetDesca("SELECT SUM(SAL)  FROM FILE4_11 WHERE [TYPE] = '4' AND CODE = " & MyParn(xCode.text) & " AND MOSM = " & MyParn(xMosm.Caption), con) & "")
    nV4 = Val(GetDesca("SELECT SUM(PAY)  FROM FILE4_11 WHERE [TYPE] = '5' AND CODE = " & MyParn(xCode.text) & " AND MOSM = " & MyParn(xMosm.Caption), con) & "")
    nV5 = Val(GetDesca("SELECT SUM(PAY - SAL )  FROM FILE4_11 WHERE ([TYPE] = '7' OR [TYPE] = 'A' OR [TYPE] = 'C' ) AND CODE = " & MyParn(xCode.text) & " AND MOSM = " & MyParn(xMosm.Caption), con) & "")
    nBal = Val(GetDesca("SELECT SUM(sal - pay )  FROM FILE4_11 WHERE CODE = " & MyParn(xCode.text) & " AND MOSM = " & MyParn(xMosm.Caption), con) & "")
