...
Begin VB.Form ViewRetSalesBr2 
   ...
   End
End
Attribute VB_Name = "ViewRetSalesBr2"
...
Me.Caption = " ãÑÊÌÚÇÊ ÊãÊ ãä ÇáÝÑæÚ áãÈíÚÇÊ ÝÑÚ   " & VSTSALESRETBR.grid1.TextMatrix(VSTSALESRETBR.grid1.Row, 1) & "   "
...
cString = " SELECT STORE_ALL.CODE , STORE_ALL.DESCA " & _
            ...
            " FROM QFILE6_20_RET INNER JOIN STORE_ALL ON STORE_ALL.CODE = QFILE6_20_RET.STORE " & _
            " WHERE QFILE6_20_RET.STORE_RET = " & MyParn(pBranch_Store) & "  AND QFILE6_20_RET.[DATE] >= " & DateSq(pDateRep1) & " AND QFILE6_20_RET.[DATE] <= " & DateSq(pDateRep2) & _
            " AND QFILE6_20_RET.STORE  <> " & MyParn(pBranch_Store) & " AND T_BOON < 0 " & _
