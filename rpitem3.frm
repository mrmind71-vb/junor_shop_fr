...
temptable.Open "temp", contemp, adOpenKeyset, adLockOptimistic, adCmdTable
    ...
    cField1 = myiif("type = '6'  ", "([OUT])  ") & " AS Q_SAL , "
    cField2 = myiif("type = '6'  ", "([OUT]* FILE1_10.COST ) ") & " AS COST_SAL  "
    cString = "SELECT MODELFACT,FILE1_10.ITEM , FILE1_10.FACT , FILE1_10.MODELNO , FILE1_10.MODEL , FILE1_10.PRICE , FILE1_10.COST , FILE1_10.DESCA , FILE1_10.SCAL , FILE1_10.COLOR , FILE1_10.C_SCAL , FILE1_10.C_COLOR , FILE1_10.MOSM , FACT.DESCA AS FACTDESCA , FILE1_10.CODE , " & _
                cField1 & cField2 & _
          " FROM (FILE1_10 INNER JOIN FILE1_11 ON FILE1_10.ITEM = FILE1_11.ITEM) LEFT JOIN FACT ON FILE1_10.[FACT] = FACT.CODE WHERE TYPE = '6'  "

If IsDate(xdate1.text) Then
    cString = cString & turn(cString) & " date >= " & DateSq(xdate1.text)
    aHeader(0) = "[" & "„‰ : " & xdate1.text & "]"
End If

If IsDate(xDate2.text) Then
    cString = cString & turn(cString) & " date <= " & DateSq(xDate2.text)
    aHeader(0) = "[" & "Õ Ì : " & xDate2.text & "]"
End If
...
If Trim(xFact.BoundText) <> "" Then
    cString = cString & turn(cString) & "File1_00.[FACT] = " & MyParn(xFact.text)
...
With sourcetable
    Do Until .EOF
        ...
        temptable!val3 = Val(GetDesca("select sum([in]-[out]) from file1_11 where item = " & !Item, con))
        temptable!val2 = !q_sal
        
        temptable!STR7 = " ›’Ì·Ï „»Ì⁄«  ·„Ê”„  " & xMosm.text & " ··„Ê—œ " & xCode.text
        temptable!str8 = TurnValue(retHeader(aHeader, 0, 5))
        temptable.Update
      .MoveNext
    Loop
End With
    main.Report1.ReportFileName = App.Path & "\Reports\R_SALModel.rpt"
    contemp.BeginTrans
    contemp.CommitTrans
...
If Not IsDate(xdate1.text) And Trim(xDate2.text) <> "" Then
    MsgBox "«· «—ÌŒ €Ì— ’ÕÌÕ"
    Exit Function
End If
MYVALID = True
