...
Do Until GrdTable.EOF
    ...
    grid1.TextMatrix(grid1.Rows - 1, 2) = IIf(GrdTable!ISDISC, 1, 0)
    GrdTable.MoveNext
Loop
grid1.AddItem ""
End Sub
...
If grid1.TextMatrix(Row, 0) <> "" Then
    con.Execute "update FILE1_50G " & _
                "  SET DESCA = " & addstring(grid1.TextMatrix(Row, 1)) & _
                ", ISDISC = " & Val(grid1.TextMatrix(Row, 2)) & _
                " WHERE code = " & MyParn(grid1.TextMatrix(Row, 0))
Else
    If Trim(grid1.TextMatrix(Row, 1)) <> "" Then
        NNEW = IncRec(GetDesca("Select max(cast(code as int)) from FILE1_50G", con))
        grid1.TextMatrix(Row, 0) = RetZero(NNEW, 3)
        con.Execute "INSERT INTO FILE1_50G ( CODE , DESCA , ISDISC )" & _
                    "VALUES( " & _
...
If KeyCode = 46 Then
    If Trim(grid1.TextMatrix(grid1.Row, 0)) <> "" Then
        If MsgBox("Remove Row ?? Are you sure", vbYesNo) = vbYes Then
            con.BeginTrans
            con.Execute "Delete from FILE1_50G  where CODE = " & MyParn(grid1.TextMatrix(grid1.Row, 0))
            con.CommitTrans
...
cString = "SELECT  *  from FILE1_50G order by code"
