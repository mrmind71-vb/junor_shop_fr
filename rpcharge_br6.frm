...
cString = "Select Sum(VAL1) AS VAL1 , SUM(VAL2) AS VAL2 , flag_desca , SUBGR , charge2 " & _
          " from ACC_MOVE_STORE WHERE VAL1 + VAL2 <> 0 "
If XACC.BoundText <> "" Then
    cString = cString & " AND ACC_MOVE_STORE.ACC = " & MyParn(cAcc)
    aHeader(2) = "[ ≈Ã„«·Ï Õ”«» " & XACC.text & "]"
If XSTORE.BoundText <> "" Then
    cString = cString & " AND ACC_MOVE_STORE.store = " & MyParn(XSTORE.BoundText)
    aHeader(2) = aHeader(2) & "[ ›—⁄ " & XSTORE.text & "]"
End If
End If
If IsDate(xDate1.text) Then
    cString = cString & " AND ACC_MOVE_STORE.date  >= " & DateSq(xDate1.text)
    aHeader(0) = "[" & BetweenString(Format(xDate1.text, "d-m-yyyy"), Format(xdate2.text, "d-m-yyyy")) & "]"
End If
If IsDate(xdate2.text) Then
    cString = cString & " AND ACC_MOVE_STORE.date <= " & DateSq(xdate2.text)
     aHeader(0) = "[" & BetweenString(Format(xDate1.text, "d-m-yyyy"), Format(xdate2.text, "d-m-yyyy")) & "]"
End If
...
contemp.BeginTrans
contemp.CommitTrans
Main.REPORT1.ReportFileName = App.Path & "\Reports\charge6.rpt"
...
data3.RecordSource = "SELECT * FROM FILE0_40 WHERE ISONEST2 = 1 ORDER BY CODE  "
